...
bool StreamDataSender::send_bg(){
	...
	success = Helper::read_file_into_byte_arr( Helper::get_share_path() + "bg-checker.png", html );
	if(!success){
		return false;
	}
...
bool StreamDataSender::send_favicon(){
	...
	success = Helper::read_file_into_byte_arr( Helper::get_share_path() + "favicon.ico", arr );
